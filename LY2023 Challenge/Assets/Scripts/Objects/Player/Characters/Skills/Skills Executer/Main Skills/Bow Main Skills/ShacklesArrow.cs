    ...
    public class ShacklesArrow : BowSkill
    {
        ...
        public override IEnumerator Execute(SkillsManager skillsManager, int skillIndex)
        {
            if (this.Arrows.Count > 0)
            {
                ...
                this.Arrows[this.InactiveArrowIndex].GetComponent<ArrowController>().SetUpShacklesArrow(this, this.ProjectilesPositions[index], mousePos, this.ProjectileLifeTime, this.ProjectileSpeed, this.AttributesManager.PhysicalDamage * 2.5f, this.AttributesManager.PhysicalPierce);
        ...
        {
            PlayerManager.Instance.StartCoroutine(this.ShacklesEffect(position));
        }
