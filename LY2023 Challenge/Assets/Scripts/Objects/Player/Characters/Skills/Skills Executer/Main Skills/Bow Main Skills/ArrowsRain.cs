...
namespace LY2023Challenge
{
    ...
    public class ArrowsRain : BowSkill
    {
        ...
        public override void Update()
        {
            if (this.EffectCircle.activeInHierarchy)
            {
                if ((_isSpawnedRainedArrows) && (this.Arrows.Count > 0))
                {
                    ...
                    {
                        this.Arrows[this.InactiveArrowIndex].GetComponent<ArrowController>().SetUpRainedArrow(_rSkillAreaPos, 1f, this.AttributesManager.PhysicalDamage * this.Values[0], this.AttributesManager.PhysicalPierce);
                    }
                }
            }
        }
        ...
        public override IEnumerator Execute(SkillsManager skillsManager, int skillIndex)
        {
            ...
            if (((Vector2)(mousePos - PlayerManager.Instance.Player.transform.position)).magnitude > (this.ProjectileLifeTime * this.ProjectileSpeed))
            {
        ...
        private int NumsOfRainedArchers()
        {
            ...
            {
                if ((this.Arrows[i].activeInHierarchy) && (this.Arrows[i].GetComponent<ArrowController>().TypeOfArcher == ArrowType.RainedArrow))
                {
                    _sum += 1;
                }
            }
